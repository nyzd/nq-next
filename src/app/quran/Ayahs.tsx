...
import { LoadingIcon, P, Container, Row, H2, Spacer } from "@yakad/ui";
...
export function Ayahs({ takhtitsAyahsBreakers, surahs }: AyahsProps) {
    ...
    const [loadedAyahTexts, setLoadedAyahTexts] = useState<
        Map<string, AyahType>
    >(new Map());
    ...
    const loadVisibleAyahs = useCallback(
        async (visibleKeys: string[]) => {
            ...
            const keysToLoad = visibleKeys.filter(
                (key) =>
                    !loadedAyahTextsRef.current.has(key) &&
                    !loadingAyahsRef.current.has(key)
            );
            ...
            try {
                ...
                const loadPromises = Array.from(surahGroups.entries()).map(
                    async ([surahNumber, ayahNumbers]) => {
                        ...
                        try {
                            const ayahs = await getAyahsBySurah(
                                "hafs",
                                surah.uuid,
                                count,
                                minAyah - 1
                            ...
                            ayahs.forEach((ayah) => {
                                ...
                                if (keysToLoad.includes(key)) {
                                    setLoadedAyahTexts((prev) =>
                                        new Map(prev).set(key, ayah)
                                    );
                                }
                            });
                        } catch (error) {
                            console.error(
                                `Error loading ayahs for surah ${surahNumber}:`,
                                error
                            );
                        }
                    }
    ...
    const handleRowsRendered = useCallback(
        ({
            ...
            const end = Math.min(
                takhtitsAyahsBreakers.length - 1,
                stopIndex + buffer
            );
    ...
    const rowRenderer = useCallback(
        ({
            ...
            const isNewPage =
                index === 0 ||
                (ayah.page &&
                    takhtitsAyahsBreakers[index - 1]?.page !== ayah.page);

            return (
                <div key={key} style={style}>
                    {isNewPage && ayah.page && (
                        <PageDivider pagenumber={ayah.page} />
                    )}
                    <div
                    ...
                    >
                        {isLoading ? (
                            ...
                            <div>
                                ...
                                {(!loadedAyah.surah ||
                                    !loadedAyah.surah?.bismillah?.is_ayah) && (
                                    <Ayah
                            ...
                            >
                                Takhtit - Surah {ayah.surah}, Ayah {ayah.ayah}{" "}
                                (Page {ayah.page})
                            </div>
                        )}
                    </div>
